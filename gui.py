...
class App(tk.Tk):  
    ...
    def save_feature(self):
        ...
        filename = asksaveasfile(defaultextension = '.png',filetypes=files) 
        if filename is None:
            return    
        self.img_fig.savefig(filename.name) 
        tk.messagebox.showinfo(message="Plots saved to "+str(filename.name))
        filename.close()
    def save_canvas(self): 
        #save current canvas 
        files = [('All Files', '*.*'),  ('JPG images', '*.jpg') ]
        filename = asksaveasfile(mode='w', defaultextension = '.jpg',filetypes=files) 
        if filename is None:
            return   
        ...
        print(filename.name)
        tk.messagebox.showinfo(message="Image saved to "+str(filename.name))
        filename.close()
...
app = App()
app.title("Character Recognition Visualizer")
app.mainloop()